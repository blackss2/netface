...
<pre>
This is golang simple network interface.
It contains three pkg cache, store.(registry is planned only)

When you use cache, you can use get/set/expire cache(redis, memcached, go-cache) via same interface.
When you use store, you can use document store(only rethink yet) via same interface.
</pre>
